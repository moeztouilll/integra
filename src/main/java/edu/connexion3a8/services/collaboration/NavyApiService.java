...
public class NavyApiService {
            ...
            try {
                ...
                if (response.statusCode() >= 200 && response.statusCode() < 300) {
                    ...
                    if (choices.length() > 0) {
                        ...
                        return new JSONObject(cleanJsonResponse(content));
                    }
                } else {
                    System.err.println("API Error: " + response.statusCode() + " - " + response.body());
                    throw new RuntimeException("API Error: " + response.statusCode());
    ...
    public CompletableFuture<List<Long>> searchProductsByText(String userRequirement, List<Product> catalog) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                JSONObject payload = new JSONObject();
                payload.put("model", "gpt-4.1-nano");
                ...
                for (Product p : catalog) {
                    sb.append(String.format(
                            ...
                            p.getCategory() != null ? p.getCategory() : "Unknown",
    ...
    public CompletableFuture<List<Long>> searchProductsByImage(File imageFile, List<Product> catalog) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                JSONObject payload = new JSONObject();
                payload.put("model", "gpt-4.1-nano");
    ...
    public CompletableFuture<String> compareProducts(Product p1, Product p2) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                String prompt = String.format(
                        "Compare ces deux produits de manière professionnelle en français.\n" +
                                ...
                                "IMPORTANT: N'utiliser JAMAIS d'étoiles (*) ou de symboles markdown. Utiliser des tirets (-) simples pour les listes. "
                                +
    ...
    public CompletableFuture<String> chatAboutProducts(Product p1, Product p2, String query) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                String prompt = String.format(
                        "Tu es un expert en produits. Réponds à la question suivante en te basant sur ces deux produits :\n"
                                ...
                                "2. %s: %s\n\n" +
                                "Question: %s\n\n" +
                                "Réponds de manière concise et utile en français. N'utiliser JAMAIS de symboles markdown comme les étoiles (**).",
